...
if netstat -tulpn 2>/dev/null | grep -q
":$8200.*LISTEN"; then
SERVER_UP=true
break
else
...
yarn run cucumberjs spec/cucumber/features -- --require-module @babel/register --require spec/cucumber/steps/*.js
...
# Terminate all processes within the same process group by sending a
SIGTERM signal
kill -15 0